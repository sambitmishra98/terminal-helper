...
preprocess() {
    ...
    python3 $SAMBITMISHRA98/terminal-helper/profession/benchmark-scripts/preprocess_templater.py \
            --output benchmark_tgv.csv \
            --mesh:etype hex --mesh:order 6 \
            --config:backend_precision double \
            --mesh:dof 625000,1250000,2500000,5000000,10000000,20000000,40000000,80000000,160000000

    cd cfgs/;
    # Delete previous cfg files
    rm *.ini
    pyfr benchmark preprocess-configs --options ../benchmark_tgv.csv --config $SAMBITMISHRA98/terminal-helper/profession/benchmark-scripts/cpu_better.ini
...
arrayrun(){
    ...
    etypes=(hex )
    dofs=( 625000 1250000 2500000 5000000 10000000 20000000 40000000 80000000 160000000 )
    orders=(6 )
    precisions=(double )
        ...
        for etype in "${etypes[@]}"; do
            for dof in "${dofs[@]}"; do
                for order in "${orders[@]}"; do
                    ...
                    cfgfile="../cfgs/cpu_betterbackend_precision-${precision}_solver_order-${order}_soln-plugin-writer_basename-m_e-${etype}__m_o-${order}__c_b-${precision}__m_d-${dof}__c_s-${order}.ini"
    ...
    for precision in "${precisions[@]}"; do
        for etype in "${etypes[@]}"; do
            for dof in "${dofs[@]}"; do
                for order in "${orders[@]}"; do
                    meshfile="../meshes/etype-${etype}_order-${order}_dof-${dof}.pyfrm"
                    cfgfile="/scratch/user/u.sm121949/EFFORTS/benchmark/testbed-tgv/Mixed/lb.ini"
                    if [[ -e "$meshfile" && -e "$cfgfile" ]]; then
                        echo "Submit $(basename "$cfgfile")"
        ...
        for etype in "${etypes[@]}"; do
            for dof in "${dofs[@]}"; do
                for order in "${orders[@]}"; do
                    ...
                    if [[ -e "$meshfile" && -e "$cfgfile" ]]; then
                        echo "Submit $(basename "$cfgfile")"
                        sbatch "$SAMBITMISHRA98/terminal-helper/profession/benchmark-scripts/sbatches/templateg2o180.LaunchMixed" "$meshfile" "$cfgfile"
                    else
                        echo "Skip  $(basename "$cfgfile") (missing mesh or cfg)"
                    fi
                done
